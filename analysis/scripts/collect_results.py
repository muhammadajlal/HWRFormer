#!/usr/bin/env python3
"""Collect every HWRFormer result needed by paper 2 into one staging JSON.

Reads `best.character_error_rate` / `best.word_error_rate` from
train_*.json across 5 folds per experiment, computes mean and std,
and writes the result to HWRFormer.json. Treat the output as the
...
Run from anywhere:
    python collect_results.py [--out path]
"""
...
def main() -> None:
    ...
    out["table1_onhw_migration"] = {
        ...
        "HWRFormer_CTC": {
            **cell(RESULTS / "Baseline-Transformer-CTC-Matched/transformer__onhw_wi_word_rh"),
        },
        "HWRFormer_AR_no_gating": {
            **cell(RESULTS / "Baseline-AR-Ungated/ar_transformer__onhw_wi_word_rh"),
        },
        "HWRFormer_AR_elementwise": {
            **cell(RESULTS / "Baseline-AR-blconv_b/ar_transformer__onhw_wi_word_rh"),
        },
        "HWRFormer_AR_headwise": {
            **cell(RESULTS / "Baseline-AR-HeadwiseGating/ar_transformer__onhw_wi_word_rh"),
        },
    }
    ...
    for variant_key, root in [
        ("HWRFormer_AR_elementwise", "Baseline-AR-blconv_b"),
        ("HWRFormer_AR_headwise",    "Baseline-AR-HeadwiseGating"),
    ]:
        out["table2_private"][variant_key] = {
            "private_words": cell(RESULTS / root / "ar_transformer__wi_word_hw6_meta"),
            "private_sent":  cell(RESULTS / root / "ar_transformer__wi_sent_hw6_meta"),
        }
    ...
    # ----- Table 3: Hybrid ablation (OnHW-WI), λ_ctc = 0.1 (optimum from λ sweep) -----
    out["table3_hybrid"] = {
        "AR_only":         cell(RESULTS / "Baseline-AR-blconv_b/ar_transformer__onhw_wi_word_rh"),
        "hybrid_independent": cell(RESULTS / "train_element_word_hybrid_01_onhw_wi/ar_transformer__onhw_wi_word_rh"),
    ...
    BASELINE_DIR = {
        ...
        "onhw_wd_word_rh":           "Baseline-AR-blconv_b",       # may differ; check after run
        "onhw_equations_wi_word_rh": "Baseline-AR-blconv_b",
        "onhw_equations_wd_word_rh": "Baseline-AR-blconv_b",
        "wi_word_hw6_meta":          "Baseline-AR-blconv_b",
