...
1. Download the two **right-handed** OnHW-words500 archives from the page above:
   `OnHW-words500_indep.zip` (writer-independent) and `OnHW-words500_dep.zip`
   (writer-dependent); the `_L` archives are left-handed and not used. Extract
   them. Each contains the five official fold directories with the released
   pickle files (`all_x_dat_{train,val}_imu.pkl`, `all_{train,val}_gt.pkl`,
   `{train,val}_ids.pkl`).
2. Open `onhw.ipynb` (e.g. `jupyter lab` inside the `hwrformer` env) and set
   ...
   |---|---|---|---|
   | WI | folder holding the five fold dirs of `OnHW-words500_indep` | `${REPO}/data/onhw_wi_word_rh` | `True` |
   | WD | folder holding the five fold dirs of `OnHW-words500_dep` | `${REPO}/data/onhw_wd_word_rh` | `False` |
