#!/usr/bin/env python3
r"""Direct exposure-bias probe: w/ TF vs.\ w/o TF decoding.

For one saved fold checkpoint, evaluate the validation set under two
decoding regimes and report both numbers + their gap:

  * w/  TF : feed the ground-truth prefix at every step (per-position
             argmax from a single forward pass); the train-time inference
             signal.
  * w/o TF : greedy unrolling without teacher forcing, feeding the
             decoder's own previous prediction back in; the path used at
             inference time and reported in the paper's main tables.

The within-checkpoint jump from w/ TF to w/o TF is the prefix-drift cost
- the direct exposure-bias signal reported in Table 3 of the paper.

Output: a JSON next to the checkpoint with
    {tf: {cer, wer, n}, wo_tf: {cer, wer, n}, gap: {cer_diff, wer_diff}}
...
Sweep across folds and training conditions with a shell loop; aggregate
the resulting JSONs to reproduce Table 3.
"""
...
from hwrformer.evaluate import evaluate
from hwrformer.model import BaseModel, DualHeadModel
from hwrformer.training.utils import build_ar_batch
from hwrformer.utils import expand_cfg_paths


# ─────────────────────────────────────────────────────────────────────────
# Helpers (inlined here to keep the script self-contained)
# ─────────────────────────────────────────────────────────────────────────

def setup_tokenizer(cfg: dict):
    """Character-mode vocabulary and special-token IDs.

    Returns (tok, vocab_dec, PAD_ID, BOS_ID, EOS_ID). HWRFormer is
    character-only; tok is always None.
    """
    AR_MODE = cfg["arch_de"] in {"ar_transformer"}
    base = int(len(cfg["categories"]))
...
def _strip_module_prefix(state_dict: dict) -> dict:
    """Remove an optional `module.` prefix from a DataParallel-saved state dict."""
...
def build_dual_or_base(cfg: dict, vocab_dec, PAD_ID, BOS_ID, EOS_ID):
    """Build BaseModel or DualHeadModel directly from the YAML cfg."""
...
def decode_ids(ids: list, categories: list, pad_id: int, bos_id: int, eos_id: int) -> str:
    """Decode a list of integer IDs to a string. Stop at EOS; drop BOS/PAD."""
    out = []
    for t in ids:
        if t == eos_id:
            break
        if t in (pad_id, bos_id):
            continue
        if 0 <= t < len(categories) and t != 0:  # index 0 is reserved (CTC blank)
            out.append(categories[t])
    return "".join(out)


@torch.no_grad()
def run_tf_eval(model, loader, cfg, device, max_samples):
    """Teacher-forced decoding: feed ground-truth prefix; argmax per position."""
    ...
    for x, y, len_x, len_y in loader:
        ...
        # DualHeadModel returns a dict {ar_logits, ctc_logits, enc_lengths};
...
@torch.no_grad()
def run_wo_tf_eval(model, loader, cfg, device, max_samples):
    """w/o TF: greedy decoding that feeds the model's own previous prediction."""
    PAD_ID, BOS_ID, EOS_ID = cfg["PAD_ID"], cfg["BOS_ID"], cfg["EOS_ID"]
    chars = cfg["categories"]
    ...
    for x, y, len_x, len_y in loader:
        x = x.to(device)
        B = x.size(0)
        # Label-independent generation horizon (see hwrformer.training.loops.test):
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Direct exposure-bias probe (w/ TF vs. w/o TF).")
    ...
    p.add_argument("--out", default=None, help="Output JSON path (default: alongside the YAML)")
...
def main() -> None:
    ...
    with open(args.config) as f:
        cfg = expand_cfg_paths(yaml.safe_load(f))
    if args.size_batch is not None:
        cfg["size_batch"] = args.size_batch
    device = torch.device(args.device)

    # Vocab + special IDs
    _tok, vocab_dec, PAD_ID, BOS_ID, EOS_ID = setup_tokenizer(cfg)
    cfg["PAD_ID"], cfg["BOS_ID"], cfg["EOS_ID"] = PAD_ID, BOS_ID, EOS_ID

    # Build model and load checkpoint
    model, dual_enabled = build_dual_or_base(cfg, vocab_dec, PAD_ID, BOS_ID, EOS_ID)
    ...
    # Two evaluation passes (order is arbitrary; both deterministic)
    tf = run_tf_eval(model, loader, cfg, device, args.max_samples)
    wo_tf = run_wo_tf_eval(model, loader, cfg, device, args.max_samples)

    gap = {
        "cer_diff": round(wo_tf["cer"] - tf["cer"], 6),
        "wer_diff": round(wo_tf["wer"] - tf["wer"], 6),
        "cer_diff_pp": round((wo_tf["cer"] - tf["cer"]) * 100, 4),
        "wer_diff_pp": round((wo_tf["wer"] - tf["wer"]) * 100, 4),
    }

    out_path = args.out or str(Path(args.config).with_name("eval_tf_gap.json"))
    payload = {
        "_meta": {
        ...
        "tf": tf, "wo_tf": wo_tf, "gap": gap,
    }
    Path(out_path).parent.mkdir(parents=True, exist_ok=True)
    with open(out_path, "w") as f:
        json.dump(payload, f, indent=2)
    logger.info("w/  TF  CER {:.4f}  WER {:.4f}  ({} samples)", tf["cer"], tf["wer"], tf["n_samples"])
    logger.info("w/o TF  CER {:.4f}  WER {:.4f}  ({} samples)", wo_tf["cer"], wo_tf["wer"], wo_tf["n_samples"])
